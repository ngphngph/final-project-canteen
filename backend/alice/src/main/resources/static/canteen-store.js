...
const CANTEEN_ORDER_WINDOW = {
  start: "11:00",
  end: "14:30",
  zone: "Asia/Hong_Kong",
  closedMessage:
    "目前不是訂購時段，請在訂購時段：11:00 - 14:30進行訂餐，謝謝﹗",
  /** 測試期間設為 false；上線前改回 true */
  enforced: false
};
...
function getNowMinutesInOrderZone() {
  const parts = new Intl.DateTimeFormat("en-GB", {
    timeZone: CANTEEN_ORDER_WINDOW.zone,
    hour: "2-digit",
    minute: "2-digit",
    hour12: false
...
function isOrderWindowOpen() {
  const now = getNowMinutesInOrderZone();
  const start = parseTimeToMinutes(CANTEEN_ORDER_WINDOW.start);
  const end = parseTimeToMinutes(CANTEEN_ORDER_WINDOW.end);
  return now >= start && now <= end;
}

/** @returns {boolean} 是否在訂購時段內（enforced=false 時一律通過，方便測試） */
function assertOrderWindowOpen() {
  if (!CANTEEN_ORDER_WINDOW.enforced || isOrderWindowOpen()) return true;
  alert(CANTEEN_ORDER_WINDOW.closedMessage);
  return false;
}
